...
if (!(Test-Path $consoleKey1))
{
	...
	New-Item -path $consoleKey -Name "HSUSCOMMS1.AD.HUMBOLDT.EDU" -Force
	New-ItemProperty -Path $consoleKey1 -PropertyType string -Name ManagementGroupName -Value HUMBOLDT -Force
	New-ItemProperty -Path $consoleKey1 -PropertyType DWORD -Name ConnectionTime -Value 0 -Force
}

if (!(Test-Path $consoleKey2))
{
	
	New-Item -path $consoleKey -Name "HSUSCOMMS2.AD.HUMBOLDT.EDU" -Force
	New-ItemProperty -Path $consoleKey2 -PropertyType string -Name ManagementGroupName -Value HUMBOLDT -Force
	New-ItemProperty -Path $consoleKey2 -PropertyType DWORD -Name ConnectionTime -Value 0 -Force
}

Set-ItemProperty -Path $consoleKey1 -Name ConnectionTime -Value 0 -force
Set-ItemProperty -Path $consoleKey2 -Name ConnectionTime -Value 0 -force
